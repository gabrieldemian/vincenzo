...
impl Disk {
    ...
    pub async fn run(&mut self) -> Result<(), Error> {
        ...
        'outer: loop {
            ...
            =======
                            ...
                            _ = lru_cleanup_interval.tick() => {
                                let mut to_retain = BitVec::<u8>::from_elem_general(self.last_accessed.len(), true);
        ...
        /// Compute the downloaded and valid pieces of the torrent and feed the
        /// read cache with the file handles.
        ...
        fn compute_downloaded_pieces(
            ...
            for (f, mmap) in file_handles {
                unsafe {
                    if let Err(e) =
                        mmap.unchecked_advise(UncheckedAdvice::DontNeed)
                    {
            ...
            *torrent_data_mut!(self, &info.info_hash, downloaded_per_piece)? =
            ...
            if is_err {
                return Err(Error::TorrentFilesMissing(
                    downloaded_pieces,
                    ones,
                ));
            }
        ...
        pub(crate) fn read_block(
            ...
            let absolute_offset = block_info.index * torrent_cache.piece_length
                + block_info.begin;
        ...
        pub(crate) async fn write_block(
            ...
            let _ =
                torrent_ctx.tx.send(TorrentMsg::DownloadedPiece(index)).await;
        ...
        /// Read all .torrent files from (in)complete folder, and add them to
        /// the client.
        pub async fn read_metainfos_and_add(
            ...
            while let Some(Ok(entry)) = entries.next() {
                ...
                let Ok(metainfo) = MetaInfo::from_bencode(&bytes) else {
                    continue;
                };
        ...
        fn verify_piece(
            ...
            for (file_meta, mmap) in mmaps {
                ...
                if piece_start < file_end && piece_end > file_start {
                    ...
                    // the file is too small to be read from, the torrent is
                    // likely incomplete.
                    ...
                    let available_bytes =
                        read_length.min(mmap.len() - file_offset);
        ...
        fn validate_piece(
            ...
            for file in &torrent.file_metadata {
                ...
                if piece_start <= file_end && piece_end >= file_start {
                    let overlap_start =
                        piece_start.max(file_start) - file_start;
        ...
        fn advise_piece_cold(&self, torrent: &Arc<TorrentCache>, piece: usize) {
            ...
            for file in &torrent.file_metadata {
                ...
                tokio::task::spawn_blocking(move || {
                    ...
                    let aligned_end =
                        (overlap_start + len).div_ceil(page) * page;
        ...
        fn evict_cold_pieces(&self) -> Result<(), Error> {
            ...
            for (info_hash, tracker) in self.torrents.iter() {
                ...
                for (piece_idx, &last) in tracker.0.iter().enumerate() {
                    if last != 0
                        && now.saturating_sub(last) > COLD_PIECE_THRESHOLD
                    {
        ...
        fn record_piece_access(
            ...
            tokio::task::spawn_blocking(move || {
                if let Ok(mut tracker) = piece_tracker.write() {
                    ...
                    error!(
                        "piece_tracker lock poisoned on `record_piece_access`"
                    )
                }
            });
            Ok(())
        ...
        fn is_piece_downloaded(
            ...
            let downloaded =
                torrent_data!(self, info_hash, downloaded_per_piece)?;
