...
pub async fn setup() -> Result<(SetupRes, impl FnOnce()), Error> {
    ...
    let s2 = PeerBuilder::new_seeder().build(&mut tracker).await?;
    let s3 = PeerBuilder::new_seeder().build(&mut tracker).await?;
    let s4 = PeerBuilder::new_seeder().build(&mut tracker).await?;
